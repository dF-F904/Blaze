...
class Levelling(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if message.content == f"<@!{self.bot.user.id}> help":
        

            

            await message.channel.send(f"Hey there, {message.author.mention}! My prefix is: `.`")
        ...
        if message.content == f"<@!{self.bot.user.id}>":
            ...
            await message.channel.send(f"Heyo {message.author.mention}! My prefix is: `.`")

        if message.content == f"<@!{self.bot.user.id}> prefix":
